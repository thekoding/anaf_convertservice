...
import com.kodingtech.models.D394.Declaratie394Type;
import com.microsoft.azure.functions.*;

/**
 * Azure Functions with HTTP Trigger.
 */
public class GenerateD394 {

    /**
     * This function listens at endpoint "/api/generated394". Two ways to invoke it
     * using "curl" command in bash: 1. curl -d "HTTP Body" {your host}/api/FillPdf
     * 2. curl {your host}/api/FillPdf?name=HTTP%20Query
     */
    @FunctionName("generated394")
    public HttpResponseMessage run(@HttpTrigger(name = "req", methods = {
            HttpMethod.POST }, authLevel = AuthorizationLevel.FUNCTION) HttpRequestMessage<Optional<Declaratie394Type>> request,
            final ExecutionContext context) {
        context.getLogger().info("Started generating D394 report.");
        ...
        Declaratie394Type d394 = request.getBody().get();
        String base64key = d394.getKey();
        String keyPassword = d394.getKeyPassword();
        ...
        try {
            ...
            JAXBContext jaxbContext = JAXBContext.newInstance(Declaratie394Type.class);
            Marshaller marshaller = jaxbContext.createMarshaller();
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
            marshaller.marshal(d394, xml);

            String errorMessage = "";

            Validation validator = new d394validator.Validator();
            PdfCreation creator = new d394.PdfCreator();
